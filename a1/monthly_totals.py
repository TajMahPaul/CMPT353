...
def pivot_months_pandas(data):
    ...
    totals = data.groupby(['name', 'month']).aggregate('sum').reset_index().pivot(index="name", columns="month", values="precipitation")
    counts = data.groupby(['name', 'month']).aggregate('count').reset_index().pivot(index="name", columns="month", values="precipitation")
    # ...
